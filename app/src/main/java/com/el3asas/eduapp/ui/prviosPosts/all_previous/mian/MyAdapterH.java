...
public class MyAdapterH extends RecyclerView.Adapter<MyAdapterH.MyViewHolder> implements Filterable {
    private List<Entity> Entity;
    private List<Entity> EntityFull;
    private static clickListener listener;

    public MyAdapterH(List<Entity> list) {
        Entity = list;
        this.EntityFull = new ArrayList<>(list);
    }

    public MyAdapterH(clickListener l) {
        listener=l;
    }
        ...
        return new Filter() {
            public Filter.FilterResults performFiltering(CharSequence charSequence) {
                ...
                if (charSequence == null || charSequence.length() == 0) {
                    arrayList.addAll(MyAdapterH.this.EntityFull);
                } else {
                    String trim = charSequence.toString().toLowerCase().trim();
                    for (Entity entity : MyAdapterH.this.EntityFull) {
            ...
            public void publishResults(CharSequence charSequence, Filter.FilterResults filterResults) {
                ...
                MyAdapterH.this.notifyDataSetChanged();
            }
        };
    }
